...
const blank = { pn: "", name: "", description: "", unit: "pcs", category: "" };
...
export function ProductsManager({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return query ? products.filter((product) => [product.pn, product.name, product.description, product.category].some((value) => value?.toLowerCase().includes(query))) : products;
  }, [products, search]);

  function open(product?: Product) {
    setEditing(product ?? null);
    setForm(product ? { pn: product.pn, name: product.name ?? "", description: product.description, unit: product.unit, category: product.category ?? "" } : blank);
    setImage(null);
    setError("");
  }
  ...
  async function archive(product: Product) {
    if (!confirm(`归档产品 ${product.pn}？历史报价不会受影响。`)) return;
    try {
  ...
  return (
    <>
      <header className="page-header">
        <div><h1>产品资料</h1><p>{products.length} 个在用产品，产品库不保存价格</p></div>
        ...
        <div className="table-scroll">
          {visible.length ? (
            <table className="data-table mobile-cards">
              <thead><tr><th style={{ width: 76 }}>图片</th><th style={{ width: 190 }}>P/N</th><th style={{ width: 180 }}>名称</th><th>英文描述</th><th style={{ width: 90 }}>单位</th><th style={{ width: 130 }}>分类</th><th style={{ width: 92 }}>操作</th></tr></thead>
              <tbody>{visible.map((product) => <tr key={product.id}>
                ...
                <td data-label="P/N"><strong>{product.pn}</strong></td><td data-label="名称">{product.name || "-"}</td><td data-label="英文描述" style={{ whiteSpace: "pre-line", lineHeight: 1.45 }}>{product.description}</td><td data-label="单位">{product.unit}</td><td data-label="分类">{product.category || "-"}</td>
      ...
      <Dialog open={editing !== undefined} onOpenChange={(_, data) => !data.open && setEditing(undefined)}>
        <DialogSurface><form onSubmit={save}><DialogBody><DialogTitle>{editing ? "编辑产品" : "新增产品"}</DialogTitle><DialogContent>
          <div className="form-grid">
            ...
            <Field label="分类"><Input value={form.category} onChange={(_, data) => setForm({ ...form, category: data.value })} /></Field>
